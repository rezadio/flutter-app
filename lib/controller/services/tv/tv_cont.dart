...
class TvCont extends GetxController {
  final Network _network;
  TvCont(this._network);
  ...
  void onInit() async {
    ...
    await getCookie();
    await getUid();
    listProductTv = await getProductTv();
    await getBalance();
    inProccess(false);
  }
  ...
  Future<dynamic> getProductTv() async {
    ...
    final response = await _network.post(
        url: 'eidupay/tv/getBiller', header: header, body: body);
    final bodyResponse = await response.stream.bytesToString();
    final decryptedBody = _network.decrypt(bodyResponse);
    final decodedBody = jsonDecode(decryptedBody);
    return decodedBody;
  ...
  Future<dynamic> getInqTv() async {
    ...
    final response = await _network.post(
        url: 'eidupay/tv/inqTv', header: header, body: body);
    final bodyResponse = await response.stream.bytesToString();
    final decryptedBody = _network.decrypt(bodyResponse);
    final decodedBody = jsonDecode(decryptedBody);
    return decodedBody;
  ...
  Future<dynamic> getPayTv(dataDenom, dataInq, String pin) async {
    final _packageInfo = await PackageInfo.fromPlatform();
    final type = dtUser['tipe'];
    final header = {'Cookie': cookie.toString()};
    final body = <String, String>{
    ...
    final response = await _network.post(
        url: 'eidupay/tv/payTv', header: header, body: body);
    final bodyResponse = await response.stream.bytesToString();
    final decryptedBody = _network.decrypt(bodyResponse);
    final decodedBody = jsonDecode(decryptedBody);
    return decodedBody;
  ...
  Future<dynamic> pay(inquiry, String pin) async {
    return await getPayTv(inquiry[0], inquiry[1], pin);
  }
  ...
  Future<void> _tv() async {
    ...
    var inquiryResponse = await getInqTv();
    Get.back();
    ...
    if (inquiryResponse['ACK'] == 'OK') {
      ...
      var bayar = await showCupertinoModalPopup(
          ...
          builder: (_) => confirmTv(inquiryResponse));
      if (bayar) {
        List<dynamic>? results = await Get.to(
          () => PinVerificationPage<TvCont>(pageController: this),
        ...
        if (isSuccess != null && isSuccess) {
          if (payResponse['ACK'] == 'PENDING') {
            ...
            await Get.offAllNamed(TransactionSuccessPage.route.name,
                arguments: PageArgument(
                    title: 'Tagihan TV Kabel',
                    description: 'Pembayaran TV Kabel Berhasil!',
                    biayaAdmin: 'Rp. ' + biayaAdmin.value,
                    nominal: 'Rp. ' + hargaCetak.value,
                    total: 'Rp. ' + hargaCetak.value,
                    ket1: 'Nama Pelanggan : ' + inquiryResponse['nama'],
  ...
  Widget confirmTv(data) {
    String start =
        data['periode'] != null ? data['periode'].toString().split('-')[0] : '';
    String end =
        data['periode'] != null ? data['periode'].toString().split('-')[1] : '';

    if (start != '') {
      periodeTagihan.value = start.substring(0, 2) +
          '-' +
          start.substring(2, 4) +
          '-' +
          start.substring(4, 8) +
